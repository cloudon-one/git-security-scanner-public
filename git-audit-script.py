...
def make_github_request(endpoint: str, params: Optional[Dict] = None) -> Dict:
    ...
    try:
        ...
        logger.error(f"HTTP Error making GitHub request to {url}: {e} (Status Code: {status_code})")
...
def get_paginated_results(endpoint: str, params: Optional[Dict] = None, max_pages: int = 100) -> List[Dict]:
            ...
            with open(report_file, 'r') as f:
                try:
                    ...
                    if isinstance(data, dict) and 'findings' in data:
                        ...
                        findings_count = 0
                    logger.info(f"Gitleaks scan complete. Found {findings_count} potential issues.")
            ...
            with open(report_file, 'r') as f:
                try:
                    ...
                    if 'Results' in data:
                        ...
                        logger.info(f"Trivy filesystem scan complete. Found {total_vulns} vulnerabilities.")
                    else:
        ...
        if os.path.exists(report_file):
            with open(report_file, 'r') as f:
                try:
                    data = json.load(f)
                    if 'Results' in data:
                        total_misconfigs = sum(len(result.get('Misconfigurations', [])) for result in data['Results'])
                        logger.info(f"Trivy configuration scan complete. Found {total_misconfigs} misconfigurations.")
                    else:
...
def generate_dynamic_recommendations(detailed_reports: Dict) -> Dict:
    ...
    if "gitleaks" in detailed_reports:
        ...
        if isinstance(gitleaks_data, list) and len(gitleaks_data) > 0:
            recommendations["critical"].append("🔴 CRITICAL: Secrets detected in repository. Immediately rotate all exposed credentials and review access controls.")
            recommendations["high"].append("🔴 HIGH: Implement secret scanning in CI/CD pipeline to prevent future secret commits.")
            recommendations["medium"].append("🟡 MEDIUM: Review and update .gitignore to exclude sensitive files.")
    ...
    if "trivy-fs" in detailed_reports:
        ...
        if isinstance(trivy_data, dict) and "Results" in trivy_data:
            ...
            if critical_vulns > 0:
                recommendations["critical"].append(f"🔴 CRITICAL: {critical_vulns} critical vulnerabilities detected. Update dependencies immediately.")
            if high_vulns > 0:
    ...
    if "trivy-config" in detailed_reports:
        ...
        if isinstance(trivy_config_data, dict) and "Results" in trivy_config_data:
            misconfigs = sum(len(result.get("Misconfigurations", [])) for result in trivy_config_data["Results"])
            if misconfigs > 0:
...
def generate_combined_report(owner: str, repo_slug: str, scan_results_summary: Dict, detailed_reports_data: Optional[Dict] = None, branch: str = None) -> Dict:
    ...
    actual_branch = branch or os.environ.get('GITHUB_REF_NAME') or os.environ.get('GIT_BRANCH') or os.environ.get('BRANCH_NAME')
    ...
    combined_report = {
        ...
        "executive_summary": {
            ...
            "quick_stats": {
                ...
                "trivy": {"vulnerabilities_found": trivy_vulnerabilities, "misconfigurations_found": trivy_misconfigurations}
            }
        },
        "detailed_results": detailed_reports_data,
        "recommendations": generate_dynamic_recommendations(detailed_reports_data or {}),
        "scan_details": {
            "gitleaks": {
...
def convert_json_to_html(report_data: Dict) -> str:
    ...
    print("🚨 FUNCTION CALLED: convert_json_to_html is executing!")
            ...
            if isinstance(result, dict) and result.get("Vulnerabilities"):
                ...
                for vuln in result.get("Vulnerabilities", []):
                    if isinstance(vuln, dict):
                        ...
                        installed_version = vuln.get("InstalledVersion", "Unknown version")
...
def send_org_slack_notification(org: str, consolidated_report: Dict, report_path: str) -> bool:
    ...
    try:
        exec_summary = consolidated_report.get("executive_summary", {})
        meta = consolidated_report.get("metadata", {})
        top_repos = consolidated_report.get("top_repositories", {}).get("by_overall_risk", [])
        ...
        message_blocks = [
            ...
            },
            {
                ...
                "fields": [
                    ...
                    {"type": "mrkdwn", "text": f"*Repositories Scanned:* {repos_scanned}"},
                    {"type": "mrkdwn", "text": f"*Risk Level:* {risk_emoji.get(risk_level, 'ℹ️')} {risk_level}"},
            ...
            },
            {
                ...
                "fields": [
                    ...
                    {"type": "mrkdwn", "text": f"*Vulnerabilities:* {vulnerabilities_found}"},
                    {"type": "mrkdwn", "text": f"*Misconfigurations:* {misconfigs_found}"},
                    {"type": "mrkdwn", "text": f"*Scan Date:* {meta.get('scan_date', 'N/A')}"}
                ]
            }
        ]
        ...
        if top_repos and any(repo['total_issues'] > 0 for repo in top_repos[:5]):
            top_repos_text = "\\n".join([
                ...
                for repo in top_repos[:5] if repo['total_issues'] > 0
            ])
        ...
        if response.status_code == 200 and response.json().get("ok"):
            logger.info(f"Slack notification sent successfully for organization '{org}'")
            return True
        else:
...
def scan_single_repository(owner: str, repo_slug: str, repo_path_base: str, reports_dir_base: str) -> Dict:
    """
    ...
    current_time_str = current_time_obj.strftime("%H:%M:%S UTC")
    ...
    if token and ((GITHUB_OWNER and GITHUB_REPO) or (owner and repo_slug)) : 
        ...
        logger.info(f"Cloning repository {owner}/{repo_slug} into temporary directory {scan_target_path}...")
        try:
            clone_url = f"https://{token}@github.com/{owner}/{repo_slug}.git"
            subprocess.run(["git", "clone", "--depth", "1", clone_url, scan_target_path], check=True, capture_output=True, text=True, timeout=300)
            logger.info(f"Repository {owner}/{repo_slug} cloned successfully to {scan_target_path}")
            ...
            if temp_clone_dir_obj: temp_clone_dir_obj.cleanup()
            if REPO_PATH_FROM_ARGS and os.path.isdir(REPO_PATH_FROM_ARGS):
                 logger.warning(f"Clone failed. Falling back to local path provided: {REPO_PATH_FROM_ARGS} for {owner}/{repo_slug}")
                 scan_target_path = REPO_PATH_FROM_ARGS
            else: 
                safe_remove_if_exists(repo_scan_temp_output_dir) 
                return {"error": f"Failed to clone repository {owner}/{repo_slug} and no valid local fallback: {str(e)}", "results":{}}
    else: 
        logger.info(f"No GitHub credentials for remote clone, or local scan mode. Assuming local scan for {owner}/{repo_slug} at path: {repo_path_base}")
        if not os.path.isdir(repo_path_base):
            logger.error(f"Local repository path {repo_path_base} does not exist or is not a directory.")
            safe_remove_if_exists(repo_scan_temp_output_dir)
            return {"error": f"Local repository path {repo_path_base} not found.", "results":{}}
    ...
    for tool_name in ["gitleaks", "trivy-fs", "trivy-config"]:
        logger.info(f"--- Starting {tool_name} scan for {owner}/{repo_slug} on path {scan_target_path} ---")
        scan_exit_code = run_scan_command(tool_name, scan_target_path, repo_scan_temp_output_dir)
        ...
        individual_report_path = os.path.join(repo_scan_temp_output_dir, report_file_name)
        current_tool_findings_data: Any = None 
        count = 0 
        misconfig_count_trivy = 0 
        
        if os.path.exists(individual_report_path) and os.path.isfile(individual_report_path) and os.path.getsize(individual_report_path) > 0:
            try:
                ...
                if tool_name == "gitleaks":
                    count = len(current_tool_findings_data) if isinstance(current_tool_findings_data, list) else 0
                    scan_results_summary[tool_name] = {"findings_count": count, "report_file": report_file_name}
                elif tool_name == "trivy-fs":
                    vuln_count_trivy = 0
                    if isinstance(current_tool_findings_data, dict) and 'Results' in current_tool_findings_data:
                ...
                logger.info(f"{tool_name} scan for {owner}/{repo_slug} processed successfully: {count} main issues found" +
                            (f", {misconfig_count_trivy} misconfigs" if tool_name == "trivy-config" and misconfig_count_trivy > 0 else ""))
            except json.JSONDecodeError as e:
                logger.error(f"Error decoding JSON from {tool_name} report {individual_report_path} for {owner}/{repo_slug}: {e}")
                detailed_reports_data[tool_name] = [] if tool_name == "gitleaks" else {"Results": []}
                scan_results_summary[tool_name] = {"error": "JSON decode error", "findings_count":0, "vulnerabilities_count":0, "misconfigurations_count":0} 
            except Exception as e: 
                logger.error(f"Error processing {tool_name} report {individual_report_path} for {owner}/{repo_slug}: {e}")
                detailed_reports_data[tool_name] = [] if tool_name == "gitleaks" else {"Results": []}
                scan_results_summary[tool_name] = {"error": str(e), "findings_count":0, "vulnerabilities_count":0, "misconfigurations_count":0}
        else: 
            logger.warning(f"{tool_name} report {individual_report_path} still not found or empty after scan command for {owner}/{repo_slug}.")
            if tool_name == "gitleaks":
                detailed_reports_data[tool_name] = []
            elif tool_name in ["trivy-fs", "trivy-config"]:
                detailed_reports_data[tool_name] = {"Results": []}
            scan_results_summary[tool_name] = {"error": "Report not found or empty post-scan", "findings_count":0, "vulnerabilities_count":0, "misconfigurations_count":0}
    ...
    logger.debug(f"Cleaning up temporary scan output directory: {repo_scan_temp_output_dir}")
    safe_remove_if_exists(repo_scan_temp_output_dir)
    ...
    high_issues = min(trivy_vulnerabilities, 10) + (trivy_misconfigurations if trivy_misconfigurations > 5 else 0)
    medium_issues = max(0, trivy_vulnerabilities - 10) + (trivy_misconfigurations if trivy_misconfigurations <= 5 and trivy_misconfigurations > 0 else 0)
    ...
    final_combined_report_data = generate_combined_report(owner, repo_slug, scan_results_summary, detailed_reports_data)
    ...
    print(f"🔥 HTML CONTENT CONTAINS 'infrastructure-as-code': {'infrastructure-as-code' in html_content}")
    ...
    logger.info(f"Scan completed for {owner}/{repo_slug}. Reports saved to {final_reports_repo_dir}")
    return {"success": True, "results": scan_results_summary, "combined_report": final_combined_report_data}

def generate_consolidated_org_report(org: str, aggregated_repo_summaries: List[Dict], overall_totals: Dict, reports_dir_base: str) -> Dict:
    ...
    for repo_data in aggregated_repo_summaries:
        if "combined_report" in repo_data:
            ...
            repo_name = repo_data["combined_report"].get("metadata", {}).get("repository", "unknown")
            ...
            repo_rankings.append({
                ...
                "risk_score": get_risk_score(risk_level, total_issues, secrets_found)
            })
    
    # Sort repositories by different criteria
    top_by_risk = sorted(repo_rankings, key=lambda x: (-x["risk_score"], -x["total_issues"]))[:10]
    top_by_secrets = sorted(repo_rankings, key=lambda x: -x["secrets_found"])[:10]
    top_by_vulnerabilities = sorted(repo_rankings, key=lambda x: -x["vulnerabilities_found"])[:10]
    top_by_misconfigs = sorted(repo_rankings, key=lambda x: -x["misconfigurations_found"])[:10]
    
    # Calculate overall risk level
    total_org_issues = overall_totals["gitleaks_findings"] + overall_totals["trivy_vulnerabilities"] + overall_totals["trivy_misconfigurations"]
    org_risk_level = "CRITICAL" if overall_totals["gitleaks_findings"] > 0 or total_org_issues > 100 else \
                     "HIGH" if total_org_issues > 50 else \
                     "MEDIUM" if total_org_issues > 0 else "LOW"
    ...
    consolidated_report = {
        ...
        "executive_summary": {
            ...
            "critical_repos": len([r for r in repo_rankings if r["risk_level"] == "CRITICAL"]),
            "high_risk_repos": len([r for r in repo_rankings if r["risk_level"] == "HIGH"]),
            "medium_risk_repos": len([r for r in repo_rankings if r["risk_level"] == "MEDIUM"]),
            "low_risk_repos": len([r for r in repo_rankings if r["risk_level"] == "LOW"])
        },
        "top_repositories": {
            "by_overall_risk": top_by_risk,
            "by_secrets": [r for r in top_by_secrets if r["secrets_found"] > 0],
            "by_vulnerabilities": [r for r in top_by_vulnerabilities if r["vulnerabilities_found"] > 0],
            "by_misconfigurations": [r for r in top_by_misconfigs if r["misconfigurations_found"] > 0]
        },
...
def get_risk_score(risk_level: str, total_issues: int, secrets_found: int) -> int:
    ...
    base_score = {"CRITICAL": 1000, "HIGH": 100, "MEDIUM": 10, "LOW": 1, "INFO": 0}.get(risk_level, 0)
    return base_score + total_issues + (secrets_found * 50)  # Secrets weighted heavily

def generate_org_recommendations(overall_totals: Dict, repo_rankings: List[Dict]) -> Dict:
...
def scan_organization_repositories(org: str, repo_path_base_for_local: str, reports_dir_base: str) -> Dict:
    ...
    logger.info(f"Found {len(repos)} repositories in organization, will scan up to {len(repos_to_scan)} (MAX_REPOS={MAX_REPOS})")
    
    aggregated_repo_summaries = []
    overall_totals = {"gitleaks_findings": 0, "trivy_vulnerabilities": 0, "trivy_misconfigurations": 0}
    
    for i, repo in enumerate(repos_to_scan):
        repo_slug = repo.get("name", "unknown")
        logger.info(f"--- Starting scan for org repo {i+1}/{len(repos_to_scan)}: {org}/{repo_slug} ---")
        repo_summary_data = scan_single_repository(org, repo_slug, repo_path_base_for_local, reports_dir_base)
        ...
        if "success" in repo_summary_data and repo_summary_data["success"]:
            ...
            overall_totals["gitleaks_findings"] += results.get("gitleaks", {}).get("findings_count", 0)
            overall_totals["trivy_vulnerabilities"] += results.get("trivy-fs", {}).get("vulnerabilities_count", 0)
            overall_totals["trivy_misconfigurations"] += results.get("trivy-config", {}).get("misconfigurations_count", 0)
    ...
    org_summary_file_path = os.path.join(org_summary_dir, "organization-scan-summary.json")
    ...
    consolidated_report = generate_consolidated_org_report(org, aggregated_repo_summaries, overall_totals, reports_dir_base)
    
    # Save consolidated report as JSON
    consolidated_json_path = os.path.join(org_summary_dir, "consolidated-security-report.json")
    ...
    consolidated_html_path = os.path.join(org_summary_dir, "consolidated-security-report.html")
    ...
    logger.info(f"Organization scan for '{org}' completed. Aggregated summary at {org_summary_file_path}")
...
def scan_user_repositories(owner: str, repo_path_base_for_local: str, reports_dir_base: str) -> Dict:
    ...
    repos_to_scan = repos[:MAX_REPOS] if len(repos) > MAX_REPOS else repos
    logger.info(f"Found {len(repos)} repositories for user, will scan up to {len(repos_to_scan)} (MAX_REPOS={MAX_REPOS})")
    ...
    for i, repo in enumerate(repos_to_scan):
        repo_slug = repo.get("name", "unknown")
        logger.info(f"--- Starting scan for user repo {i+1}/{len(repos_to_scan)}: {owner}/{repo_slug} ---")
        repo_summary_data = scan_single_repository(owner, repo_slug, repo_path_base_for_local, reports_dir_base)
    ...
    logger.info(f"User scan for '{owner}' completed. Aggregated summary at {user_summary_file_path}")
    
    total_user_issues = overall_totals["gitleaks_findings"] + overall_totals["trivy_vulnerabilities"] + overall_totals["trivy_misconfigurations"]
    user_risk_level = "CRITICAL" if overall_totals["gitleaks_findings"] > 0 or total_user_issues > 100 else \
...
def process_scan_reports(gitleaks_report_path: str = None, trivy_report_path: str = None, osv_report_path: str = None) -> Dict:
    ...
    if gitleaks_report_path and os.path.exists(gitleaks_report_path):
        try:
            ...
            scan_results_summary["gitleaks"] = {"findings_count": count, "report_file": "gitleaks-report.json"}
    ...
    if trivy_report_path and os.path.exists(trivy_report_path):
        try:
            ...
            logger.info(f"Processed Trivy report: {vuln_count} vulnerabilities and {misconfig_count} misconfigurations found")
        except Exception as e:
            logger.error(f"Error processing Trivy report: {e}")
            detailed_reports_data["trivy-fs"] = {"Results": []}
            scan_results_summary["trivy-fs"] = {"vulnerabilities_count": 0, "misconfigurations_count": 0, "error": str(e)}
    return {
        "scan_results_summary": scan_results_summary,
        "detailed_reports_data": detailed_reports_data
    }

def save_final_reports(combined_report: Dict, reports_output_dir_base: str, owner: str, repo_name: str):
...
def detect_repository_info(repo_path: str) -> tuple:
                    ...
                    try:
                        ...
                        if 'github.com' in remote_url:
                            ...
                            if path and '/' in path:
                                ...
                                if not repo_name:
                                    repo_name = parts[1]
                                logger.info(f"Detected repository from git remote: {owner}/{repo_name}")
                    except subprocess.SubprocessError as e:
                    ...
                    try:
                        ...
                        if branch != 'HEAD':
                            ...
                            try:
                                ...
                                logger.info(f"Detected branch from git describe: {branch}")
    ...
    logger.info(f"Final detected repository info - Owner: {owner}, Repo: {repo_name}, Branch: {branch}")
    return owner, repo_name, branch

def generate_summary_for_local_scan(local_repo_reports_path: str, reports_output_dir_base: str) -> Dict:
    ...
    if os.path.exists(gitleaks_report) or os.path.exists(trivy_report):
        ...
        detected_owner, detected_repo, detected_branch = detect_repository_info(scan_path)
        
        # Generate the combined report with detected info
        combined_report = generate_combined_report(detected_owner, detected_repo, scan_results_summary, detailed_reports_data, detected_branch)
        
        # Save the final reports
        save_final_reports(combined_report, reports_output_dir_base, detected_owner, detected_repo)
    ...
    if not summary_files:
        logger.warning("No summary.json files found in local reports path and no raw scan reports found")
    ...
    for summary_file in summary_files:
        try:
            ...
            overall_totals["gitleaks_findings"] += results.get("gitleaks", {}).get("findings_count", 0)
            overall_totals["trivy_vulnerabilities"] += results.get("trivy", {}).get("vulnerabilities_count", 0)
            overall_totals["trivy_misconfigurations"] += results.get("trivy", {}).get("misconfigurations_count", 0)
    ...
    overall_summary_path = os.path.join(reports_output_dir_base, "local-scan-overall-summary.json")
    ...
    total_local_issues = overall_totals["gitleaks_findings"] + overall_totals["trivy_vulnerabilities"] + overall_totals["trivy_misconfigurations"]
    local_risk_level = "CRITICAL" if overall_totals["gitleaks_findings"] > 0 or total_local_issues > 100 else \
                       "HIGH" if total_local_issues > 50 else \
                       "MEDIUM" if total_local_issues > 0 else "LOW"
    
    # Slack notification removed
    
    return {"success": True, "repositories_scanned": len(aggregated_summaries), "total_issues": total_local_issues}
...
def check_pipeline_exit_conditions(combined_report: Dict) -> bool:
    ...
    if total_issues > 50:
        logger.error(f"High number of security issues found ({total_issues}). Pipeline will exit.")
        return True
    
    return False

def fail_pipeline_on_critical_findings(combined_report: Dict, exit_code: int = 1) -> None:
...
def main():
    ...
    parser.add_argument("--action", choices=["scan-single-repo", "scan-org", "scan-user", "generate-summary"], 
                       default="scan-single-repo", help="Action to perform")
    parser.add_argument("--repo-path", help="Path to local repository (for local scans)")
    parser.add_argument("--reports-dir", default="/scan_target/scan_reports", help="Directory for reports")
    ...
    try:
        if args.action == "scan-single-repo":
            if not GITHUB_OWNER or not GITHUB_REPO:
                logger.error("GITHUB_OWNER and GITHUB_REPO environment variables must be set for single repository scan")
                sys.exit(1)
            
            result = scan_single_repository(GITHUB_OWNER, GITHUB_REPO, REPO_PATH_FROM_ARGS or "/scan_target", reports_dir)
            ...
            if not GITHUB_ORG:
                logger.error("GITHUB_ORG environment variable must be set for organization scan")
                sys.exit(1)
            
            result = scan_organization_repositories(GITHUB_ORG, REPO_PATH_FROM_ARGS or "/scan_target", reports_dir)
            ...
            if not GITHUB_OWNER:
                logger.error("GITHUB_OWNER environment variable must be set for user repository scan")
                sys.exit(1)
            
            result = scan_user_repositories(GITHUB_OWNER, REPO_PATH_FROM_ARGS or "/scan_target", reports_dir)
            ...
            if not REPO_PATH_FROM_ARGS:
                logger.error("--repo-path must be specified for generate-summary action")
                sys.exit(1)
